  ...
  return <section className="page-layout members-page">
    ...
    {<section className="member-onboarding">
      <div className="member-onboarding-copy">
        ...
        <button type="button" className="secondary member-refresh" onClick={props.onRefreshMembers} disabled={props.memberBusy}><RefreshCw className={props.memberBusy ? 'spin' : ''} size={16} />更新</button>
      </div>
            ...
            <div className="member-edit-line">
              ...
              {member.lineDestinations.length > 0 && <div className="member-edit-line-list">
                {member.lineDestinations.map((destination) => <div key={destination.id}>
                  ...
                  <code>{destination.destinationId}</code>
                  <button type="button" className="member-line-unlink" onClick={() => void unlinkLineDestination(member.id, destination.id)} disabled={props.memberBusy}><X size={13} />解除</button>
                </div>)}
              </div>}
              <div className="member-edit-line-manual">
                ...
                <button type="button" className="secondary" onClick={() => void saveManualLineDestination(member.id)} disabled={props.memberBusy || !editManualDestinationId.trim()}>設定</button>
              </div>
            </div>
            <div className="member-edit-actions"><button className="primary" onClick={() => void saveEdit(member.id)} disabled={props.memberBusy}><Save size={15} />保存</button><button className="secondary" onClick={() => setEditingId('')}><X size={15} />キャンセル</button></div>
          </div> : <>
...
export const TasksPage = (props: DashboardProps) => {
  ...
  return <section className="page-layout tasks-page">
    ...
    {<section className="task-role-management">
      ...
      <div className="task-role-list">{props.taskRoles.map((role) => <article key={role.id}><strong>{role.displayName}</strong><p>{role.description}</p><details><summary>名前と説明を編集</summary><label>名前<input defaultValue={role.displayName} onBlur={(change) => { const displayName = change.target.value.trim(); if (displayName && displayName !== role.displayName) void props.onUpdateTaskRole(role.id, { displayName }); }} /></label><label>説明<textarea defaultValue={role.description} onBlur={(change) => { const description = change.target.value.trim(); if (description && description !== role.description) void props.onUpdateTaskRole(role.id, { description }); }} /></label></details><label>現在の担当者<select value={props.taskRoleAssignments.find((assignment) => assignment.roleId === role.id)?.memberId ?? ''} disabled={props.busy} onChange={(change) => { if (change.target.value) props.onAssignTaskRole(role.id, change.target.value); }}><option value="">未割り当て</option>{props.taskMembers.map((member) => <option key={member.memberId} value={member.memberId}>{member.displayName}</option>)}</select></label><button type="button" className="secondary" onClick={() => void props.onDeleteTaskRole(role.id)} disabled={props.busy}>roleを削除</button></article>)}</div>
    </section>}
...
export const ConnectionsPage = (props: DashboardProps) => {
  ...
  return <section className="page-layout settings-page">
    ...
    {!settingsReady ? <section className="empty-page"><Settings size={30} /><h2>設定を読み込めません</h2><p>Googleでログインし直した後、このページを再読み込みしてください。</p></section> : <>
      ...
      <section className="settings-card"><div className="settings-card-title"><Settings size={19} /><div><h2>添付ファイルの保存先</h2><p>Google Driveのどこに添付ファイルを置くかを決めます。</p></div></div><p className="api-guide">「/」で階層を区切ります。ここで指定したフォルダの下に、メール1通ごとのフォルダを受信日と件名で作成します。Mail Automationが作成したフォルダだけを扱うため、手作業で作った同名フォルダとは別に作成されます。</p><label>保存先<input value={props.attachmentFolderPath} onChange={(event) => props.onAttachmentFolderPathChange(event.target.value)} placeholder="Mail Automation/添付ファイル" autoCapitalize="none" spellCheck={false} /></label><div className="settings-card-actions"><p className="connection-state">現在: {props.savedAttachmentFolderPath}</p><button className="primary" onClick={props.onSaveAttachmentFolderPath} disabled={props.attachmentFolderBusy || !props.attachmentFolderPath.trim() || props.attachmentFolderPath === props.savedAttachmentFolderPath}><Save size={17} />{props.attachmentFolderBusy ? '保存中…' : '保存先を保存'}</button></div></section>
...
const PromptEditor = ({ prompt, props }: { prompt: DashboardProps['prompts'][number]; props: DashboardProps }) => {
  ...
  return <article className="rule-row"><div><strong>{prompt.name}</strong><small>revision {prompt.revision}</small><p>{prompt.instructions}</p><details><summary>Promptを編集</summary><label>Prompt名<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Instructions<textarea value={instructions} onChange={(event) => setInstructions(event.target.value)} /></label><button type="button" className="secondary" disabled={props.ruleBusy} onClick={() => void props.onUpdatePrompt(prompt.id, { name, instructions })}>{props.ruleBusy ? '保存中…' : 'Promptを保存'}</button></details></div><button type="button" className="secondary" disabled={props.ruleBusy} onClick={() => void props.onDeletePrompt(prompt.id)}>Promptを削除</button></article>;
};
...
const AgentRuleEditor = ({ rule, props }: { rule: DashboardProps['agentRules'][number]; props: DashboardProps }) => {
  ...
  return <details className="rule-destination-editor"><summary>Execution Mode・許可リストを編集</summary><label>Execution Mode<select value={rule.executionMode} disabled={props.ruleBusy} onChange={(event) => void props.onUpdateAgentRule(rule.id, { executionMode: event.target.value as 'read_only' | 'approval' | 'unattended' })}><option value="read_only">Read only</option><option value="approval">Approval</option><option value="unattended">Unattended</option></select></label><DestinationListChoices legend="許可されたCalendar Recipient Lists" lists={recipientLists} selectedIds={recipientIds} onChange={setRecipientIds} /><DestinationListChoices legend="許可されたLINE Destination Lists" lists={lineLists} selectedIds={lineIds} onChange={setLineIds} /><button type="button" className="secondary" disabled={props.ruleBusy} onClick={() => void props.onUpdateAgentRule(rule.id, { permittedRecipientListIds: recipientIds, permittedLineListIds: lineIds })}>許可リストを保存</button></details>;
};
...
const AgentRunHistory = (props: DashboardProps) => {
  ...
  return <section className="rules-list"><div className="rules-list-title"><h2>Run Transcripts</h2><span>{props.agentRuns.length}件</span></div>{props.agentRuns.map((run) => <article className="rule-row" key={run.id}><div><strong>{props.agentRules.find((rule) => rule.id === run.agentRuleId)?.name ?? run.agentRuleId}</strong><small>{run.outcome} ・ {run.model} ・ tools {run.toolCallCount} ・ tokens {run.tokens}</small></div><button type="button" className="secondary" disabled={props.ruleBusy} onClick={() => props.onLoadAgentTranscript(run.id)}>{props.ruleBusy ? '読み込み中…' : 'Run Transcriptを読む'}</button></article>)}{props.agentTranscript && <article className="test-card"><div><p>RUN TRANSCRIPT</p><h2>{props.agentTranscript.source.subject}</h2></div><pre>{props.agentTranscript.source.body}</pre>{props.agentTranscript.source.attachments.map((attachment) => <pre key={attachment.filename}>{attachment.filename}{'\n'}{attachment.text}</pre>)}<pre>{props.agentTranscript.finalOutput || props.agentTranscript.error}</pre>{props.proposedActions.length > 0 && <section><h3>Proposed Actions</h3>{pending.length > 1 && runId && <div><button type="button" className="primary" disabled={props.ruleBusy} onClick={() => props.onDecideProposedActionBatch(runId, 'approve')}>すべて承認</button><button type="button" className="secondary" disabled={props.ruleBusy} onClick={() => props.onDecideProposedActionBatch(runId, 'reject')}>すべて却下</button></div>}{props.proposedActions.map((action) => <article key={action.id}><strong>{action.tool}</strong><small>{action.status} ・ expires {action.expiresAt}</small><pre>{JSON.stringify(action.arguments, null, 2)}</pre>{action.status === 'pending' && <div><button type="button" className="primary" disabled={props.ruleBusy} onClick={() => props.onDecideProposedAction(action.id, 'approve')}>承認</button><button type="button" className="secondary" disabled={props.ruleBusy} onClick={() => props.onDecideProposedAction(action.id, 'reject')}>却下</button></div>}</article>)}</section>}</article>}</section>;
};
