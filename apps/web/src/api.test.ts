...
describe('Account setup client', () => {
  ...
  it('loads the Account-scoped dashboard rather than the retired global automation summary', async () => {
  ...
  it('loads the Automation Inbox through the selected Account', async () => {
  ...
  it('returns a newly created Account Rule', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({
      data: { id: 'rule-1', name: 'Announcements', state: 'draft' },
    }), { status: 201 })));
