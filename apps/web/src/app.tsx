...
import { api } from './api';
import type { AutomationStatus, AutomationSummary, AuthMe, DeliveryAuditRecord, OrganizationConnections, OrganizationDashboard, OrganizationRule } from './api';
...
export const App = () => {
  ...
  useEffect(() => {
    if (!organizationId) { setConnections(null); setOrganizationDashboard(null); setOrganizationRules([]); setDeliveryAudit([]); return; }
    void Promise.all([api.organizationConnections(organizationId), api.organizationDashboard(organizationId), api.organizationRules(organizationId), api.organizationDeliveryAudit(organizationId)]).then(([value, dashboard, rules, audit]) => {
