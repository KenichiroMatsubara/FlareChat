import { CalendarDays, CheckCircle2, CircleAlert, Eye, EyeOff, LogOut, Mail, Play, RefreshCw, Save, Settings, SlidersHorizontal } from 'lucide-react';
import { useState } from 'react';

import type { AutomationStatus, AutomationSummary, MailboxTestMatch, MailboxTestPreview, OrganizationConnections, OrganizationRule, OrganizationRuleInput } from './api';

type Page = 'automation' | 'settings' | 'rules' | 'mail-test';
...
export const Dashboard = (props: DashboardProps) => {
  const [page, setPage] = useState<Page>('automation');
  ...
  return <div className="app-shell">
    <header className="app-topbar">
      ...
      <nav aria-label="メインナビゲーション">
        <button className={page === 'automation' ? 'active' : ''} onClick={() => navigate('automation')}><Play size={16} />自動化</button>
        <button className={page === 'settings' ? 'active' : ''} onClick={() => navigate('settings')}><Settings size={16} />接続設定</button>
        <button className={page === 'rules' ? 'active' : ''} onClick={() => navigate('rules')}><SlidersHorizontal size={16} />ルール</button>
        <button className={page === 'mail-test' ? 'active' : ''} onClick={() => navigate('mail-test')}><SlidersHorizontal size={16} />メールテスト</button>
      </nav>
    ...
    <main className="app-content">
      ...
      {page === 'automation' && <section className="page-layout automation-page">
      ...
      {page === 'settings' && <section className="page-layout settings-page">
      ...
      {page === 'mail-test' && <section className="page-layout mail-test-page">
        ...
        {!settingsReady || !props.automation ? <section className="empty-page"><SlidersHorizontal size={30} /><h2>メールテストを開始できません</h2><p>Googleログインと接続設定を完了してください。</p></section> : <>
          ...
          {props.mailTestMatches.length > 0 && <section className="test-card"><div><p>2. EXTRACT EVENT</p><h2>AIで予定を抽出</h2><span>対象メールを選ぶと、内容を Gemini がプレビューします。</span></div><div className="mail-matches">{props.mailTestMatches.map((message) => <button key={message.id} className="mail-match" onClick={() => props.onPreviewMailbox(message.id)} disabled={props.mailTestBusy}><strong>{message.subject}</strong><small>{message.sender || '差出人なし'}</small></button>)}</div></section>}
          {props.mailTestPreview && <section className="test-card event-preview"><div><p>3. REVIEW AND CREATE</p><h2>予定内容を確認</h2><span>確認後にだけ Google Calendar へ作成します。</span></div><dl><dt>予定名</dt><dd>{props.mailTestPreview.event.title}</dd><dt>日時</dt><dd>{formatted(props.mailTestPreview.event.startsAt)} 〜 {formatted(props.mailTestPreview.event.endsAt)}</dd><dt>場所</dt><dd>{props.mailTestPreview.event.location || '指定なし'}</dd><dt>説明</dt><dd>{props.mailTestPreview.event.description || '指定なし'}</dd></dl><button className="primary" onClick={props.onCreateCalendarEvent} disabled={props.mailTestBusy || Boolean(props.mailTestCreatedEventId)}>{props.mailTestCreatedEventId ? 'Calendarに作成済み' : props.mailTestBusy ? '作成中…' : 'この内容で Calendar に追加'}</button>{props.mailTestCreatedEventId && <p className="dashboard-success"><CheckCircle2 size={17} />テスト予定を作成しました。</p>}</section>}
        </>}
      </section>}

      {page === 'rules' && <section className="page-layout rules-page">
