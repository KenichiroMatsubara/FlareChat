...
import { contacts, sourceMessages } from './storage/account-schema';
import { createTaskWorkflow, UNASSIGNED_TASK_ROLE } from './tasks';

const openDatabases: TestD1Database[] = [];

const seedContact = async (drizzle: ReturnType<typeof accountDatabase>): Promise<void> => {
  await drizzle.insert(contacts).values({
    id: 'member-1', accountId: 'organization-1', name: '会計担当', email: 'kaikei@example.com',
...
describe('Operational Task Role workflow', () => {
  it('keeps the role display name captured by a Task when its Account role is renamed', async () => {
    const database = createMigratedTestD1('organization');
    openDatabases.push(database);
    const drizzle = accountDatabase(database.binding);
    const workflow = createTaskWorkflow(drizzle);
    await seedContact(drizzle);
    ...
    await workflow.assignRole({ roleId: role.id, contactId: 'member-1', displayName: '会計担当' });
    ...
    await expect(workflow.list()).resolves.toEqual([
      expect.objectContaining({
        ...
        assigneeContactId: 'member-1',
        assigneeName: '会計担当',
      }),
    ]);
  });

  it('removes an Account role and its current assignment without deleting historical Tasks', async () => {
    const database = createMigratedTestD1('organization');
    openDatabases.push(database);
    const drizzle = accountDatabase(database.binding);
    const workflow = createTaskWorkflow(drizzle);
    ...
    await workflow.assignRole({ roleId: role.id, contactId: 'member-1', displayName: '会計担当' });
    await workflow.createFromSourceMessage({
      accountId: 'organization-1', sourceMessageId: 'source-2', sourceMessageSubject: '支払案内',
...
describe('Task Reassignment Review', () => {
  ...
  it('counts only the open Tasks a review would look at', async () => {
    ...
    const role = await workflow.createRole({ displayName: '会計担当', description: '支払期限を扱う' });
    await workflow.createFromSourceMessage({
      accountId: 'organization-1', sourceMessageId: 'source-3', sourceMessageSubject: '総会案内',
      extractedTasks: [
  ...
  it('moves an accepted Task onto the current holder of its new role and closes the review', async () => {
    ...
    await workflow.assignRole({ roleId: payment.id, contactId: 'member-1', displayName: '会計担当' });
    await workflow.createFromSourceMessage({
      accountId: 'organization-1', sourceMessageId: 'source-3', sourceMessageSubject: '総会案内',
    ...
    expect(applied.tasks).toEqual([expect.objectContaining({
      assigneeRoleId: payment.id, assigneeRoleName: '会計担当', assigneeContactId: 'member-1', assigneeName: '会計担当',
    })]);
    await expect(workflow.markReassignmentReviewed()).resolves.toMatchObject({ pending: false });
  });

  it('skips a Task that is already completed or names a role the Account no longer defines', async () => {
    const { workflow } = await openReview();
    const role = await workflow.createRole({ displayName: '会計担当', description: '支払期限を扱う' });
    await workflow.createFromSourceMessage({
      accountId: 'organization-1', sourceMessageId: 'source-3', sourceMessageSubject: '総会案内',
  ...
  it('returns an open Task to unassigned when no role fits it', async () => {
    ...
    expect(applied.tasks).toEqual([expect.objectContaining({
      assigneeRoleId: 'unassigned', assigneeRoleName: '未割り当て', assigneeContactId: null, assigneeName: '未割り当て',
    })]);
  });
});
