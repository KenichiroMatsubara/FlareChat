...
listRoutes.get('/organizations/:accountId/lists', accountRoute(async ({ db, accountId }) => {
  const rows = await db.select().from(lists).orderBy(asc(lists.name)).all();
  return rows.map((row) => ({ ...row, accountId }));
}));

listRoutes.post('/organizations/:accountId/lists', accountRoute<{ kind?: string; name?: string; description?: string }>(async ({ db, accountId, body }) => {
...
listRoutes.get('/organizations/:accountId/contact-lists', accountRoute(async ({ db }) => {
