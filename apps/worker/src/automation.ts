...
import { accountKeys, accounts } from './storage/control-schema';
import {
  agentRules,
...
const requireActiveAiConnection = async (database: D1Database): Promise<void> => {
  const connection = await drizzleAccountDatabase(database).select({ id: connections.id }).from(connections)
...
const accountKeyFor = async (env: Bindings, accountId: string): Promise<CryptoKey> => {
  ...
  return unwrapAccountKey({ masterKeyVersion: record.masterKeyVersion, envelope: JSON.parse(record.wrappedKeyEnvelope) }, await masterKey(env.CREDENTIAL_MASTER_KEY), accountId);
};
...
const storedInboxToken = async (
  ...
  const token = JSON.parse(await decrypt(JSON.parse(inbox.tokenEnvelope), key, `google-connection:${accountId}:automation-inbox`)) as GoogleTokenSet;
  return { key, token };
};
...
const accessTokenForInbox = async (
  ...
  const { key, token } = await storedInboxToken(env, accountId, inbox);
  ...
  const envelope = await encrypt(JSON.stringify(refreshed), key, `google-connection:${accountId}:automation-inbox`);
  await drizzleAccountDatabase(database).update(googleConnections)
...
const verifyAccountInboxCredentialWithDependencies = async (
  env: Bindings,
  accountId: string,
  database: D1Database,
  dependencies: AutomationDependencies,
  ...
  if (!inbox) return;
  try {
    await accessTokenForInbox(env, accountId, database, inbox, dependencies);
  } catch (error) {
    await recordAutomationFailure({ env, accountId, database, inbox, error, dependencies });
  }
};
...
const createAgentWritePort = (input: {
  ...
  createScheduledEvent: async (arguments_) => {
    ...
    await database.insert(events).values({
      id: eventId, accountId: input.accountId, ruleId: null, agentRuleId: input.agentRuleId,
...
export const agentWritePortForApproval = async (input: {
  ...
  const inbox = await drizzleAccountDatabase(input.database).select().from(googleConnections).where(and(
  ...
  const googleAccessToken = await accessTokenForInbox(input.env, input.accountId, input.database, inbox, productionDependencies);
...
const runMatchingAgentRules = async (input: {
  ...
  try {
    connection = await agentConnection(input.env, input.accountId, input.database);
  } catch (error) {
    connectionError = error instanceof Error ? error.message : 'Agent Rule AI Connection failed.';
  }
  const accountKey = await accountKeyFor(input.env, input.accountId);
  let failed = false;
...
const previewSchemaDraftRule = async (input: {
  ...
  const source = await readMailboxTestSourceWithGoogle(
    input.env, input.accountId, input.database, input.messageId, productionDependencies,
  );
 ...
 * whitespace — including a full-width space — collapses to one. An AccountIdentity
...
const activeAutomationInbox = async (database: D1Database): Promise<AutomationInbox> => {
  const inbox = await drizzleAccountDatabase(database).select().from(googleConnections).where(and(
    eq(googleConnections.kind, 'automation_inbox'),
    eq(googleConnections.status, 'active'),
  )).limit(1).get();
...
/** Finds recent exact-subject matches in the Account's Automation Inbox without changing Gmail state or the history boundary. */
const searchMailboxForTestWithGoogle = async (
  ...
  const accessToken = await accessTokenForInbox(env, accountId, database, await activeAutomationInbox(database), dependencies);
...
export const searchMailboxForTest = async (
  ...
  database: D1Database,
  subject: string,
): Promise<MailboxTestMatch[]> => searchMailboxForTestWithGoogle(env, accountId, database, subject, productionDependencies);

/** Reads one selected message from the Account's Automation Inbox for a server-side AI preview. */
...
const previewMailboxTestWithActiveRule = async (
  ...
  const source = await readMailboxTestSourceWithGoogle(env, accountId, database, messageId, dependencies);
...
export const readMailboxTestSource = async (
  ...
  database: D1Database,
  messageId: string,
): Promise<MailboxTestSource> => readMailboxTestSourceWithGoogle(env, accountId, database, messageId, productionDependencies);
...
const createMailboxTestCalendarEventsWithGoogle = async (
  ...
  const folderId = contents.length ? await resolveSourceMessageFolder({
    database: drizzleAccountDatabase(database),
...
/** Asks the AI for a correspondence and turns it into the plan an AccountIdentity approves. */
const planEventRefreshWithGoogle = async (
  ...
  const correspondences = inWindow.length
    ? await aiEventCorrespondence(env, accountId, database, input.events, inWindow, dependencies)
    : [];
...
const recordRefreshEffect = async (
  ...
  if (input.effect === 'updated') {
    await drizzleAccountDatabase(database).update(events).set({
  ...
  await writeRecoveryReceipt({
    ...
    accountKey: await accountKeyFor(env, accountId),
    receipt: {
      accountId,
 ...
 * Scheduled Event, deliberately overwriting Manual Overrides because an AccountIdentity
 * approved this one event's diff, and additively invites the active Contact
 * roster. A Contact the Calendar already lists — invited earlier by this same
 * exit or by ADR 0125's automated invitation — keeps whatever they answered;
 * only Contacts missing from that list are appended.
 */
const applyEventRefreshWithGoogle = async (
  ...
  const invitees = await activeContactInvitees(database);
  ...
  for (const entry of input.entries) {
    ...
    try {
      if (!entry.googleEventId) {
        ...
        await recordRefreshEffect(env, accountId, database, {
...
const correlatedTargets = async (
  ...
  const correspondences = await aiEventCorrespondence(
    env, accountId, database, candidates, targets.map((target) => target.current), dependencies,
  );
...
const planSchemaCorrelations = async (input: {
  ...
  const windowDays = input.extraction.kind === 'response'
    ? await accountResponseWindowDays(drizzleAccountDatabase(input.database))
    : null;
...
const mergeScheduledEvent = async (input: {
  ...
  await drizzleAccountDatabase(input.database).update(events).set({
    title: merged.title,
...
const applySchemaExtraction = async (
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    await db.insert(events).values({
      id: eventId, accountId: input.accountId, ruleId: rule.id, sourceMessageId,
...
export const schemaRuleEffectPortForApproval = async (input: {
  ...
  const accessToken = await accessTokenForInbox(input.env, input.accountId, input.database, inbox, productionDependencies);
...
const runAccountInbox = async (
  ...
  const accessToken = await accessTokenForInbox(env, accountId, database, inbox, dependencies);
  const db = drizzleAccountDatabase(database);
  let reprocessed = 0;
  if (reprocessSkipped) {
  ...
  do {
    ...
    for (const entry of history.history ?? []) {
      for (const message of entry.messagesAdded ?? []) {
        ...
        try {
          await processAccountMessage(dependencies, env, database, accountId, accessToken, inbox.gmailHistoryId, messageId);
        } catch (error) {
...
const runAccountAutomationWithGoogle = async (
  env: Bindings,
  accountId: string,
  database: D1Database,
  dependencies: AutomationDependencies,
  ...
  const baseline = await ensureBaselineSchemaRule(db, accountId);
  await resumeDueRuleRuns({ env, database, accountId });
  const before = await automationCounts(database);
  const run = await runAccountInbox(dependencies, env, accountId, database, inbox, baseline.repairSkipped);
...
const runEnabledAutomationsWithDependencies = async (env: Bindings, dependencies: AutomationDependencies): Promise<void> => {
  const activeAccounts = await drizzleControlDatabase(env.CONTROL_DB).select({
    id: accounts.id,
  ...
  for (const account of activeAccounts) {
    ...
    try {
      ...
      const orgDb = drizzleAccountDatabase(database.raw);
      await resumeDueRuleRuns({ env, database: database.raw, accountId: account.id });
      ...
      for (const inbox of inboxes) {
        try {
          ...
          const baseline = await ensureBaselineSchemaRule(orgDb, account.id);
          await runAccountInbox(dependencies, env, account.id, database.raw, inbox, baseline.repairSkipped);
...
/**
 * The Account Automation module's interface. HTTP and scheduled callers
...
export const createAutomation = (
  ...
  return {
    runAccount: (input: { accountId: string; database: D1Database }): Promise<AutomationSummary> =>
      runAccountAutomationWithGoogle(env, input.accountId, input.database, dependencies),
    verifyAccountInboxCredential: (input: { accountId: string; database: D1Database }): Promise<void> =>
      verifyAccountInboxCredentialWithDependencies(env, input.accountId, input.database, dependencies),
    runEnabledAccounts: (): Promise<void> => runEnabledAutomationsWithDependencies(env, dependencies),
    mailboxTest: {
      search: (input: { accountId: string; database: D1Database; subject: string }): Promise<MailboxTestMatch[]> =>
        searchMailboxForTestWithGoogle(env, input.accountId, input.database, input.subject, dependencies),
      preview: (input: { accountId: string; database: D1Database; messageId: string }): Promise<{ source: MailboxTestSource; rule: ActiveRule; extraction: MailExtraction }> =>
        previewMailboxTestWithActiveRule(env, input.accountId, input.database, input.messageId, dependencies),
      createCalendarEvents: (input: { accountId: string; database: D1Database; messageId: string; events: EventDetails[] }): Promise<{ eventIds: string[] }> =>
        createMailboxTestCalendarEventsWithGoogle(env, input.accountId, input.database, { messageId: input.messageId, events: input.events }, dependencies),
      readSource: (input: { accountId: string; database: D1Database; messageId: string }): Promise<MailboxTestSource> =>
        readMailboxTestSourceWithGoogle(env, input.accountId, input.database, input.messageId, dependencies),
      extractPackage: (input: { accountId: string; database: D1Database; source: string; attachments: SourceAttachmentContent[]; receivedAt?: string }): Promise<MailExtraction | null> =>
        extractMailboxTestPackage(env, input.accountId, input.database, input.source, input.attachments, input.receivedAt, dependencies),
      ...
      previewRefreshRequest: (input: { accountId: string; database: D1Database; messageId: string; events: EventDetails[] }): Promise<MailboxTestRefreshRequest> =>
        previewEventRefreshRequestWithGoogle(env, input.accountId, input.database, { messageId: input.messageId, events: input.events }, dependencies),
      planRefresh: (input: { accountId: string; database: D1Database; messageId: string; events: EventDetails[] }): Promise<MailboxTestRefreshPlan> =>
        planEventRefreshWithGoogle(env, input.accountId, input.database, { messageId: input.messageId, events: input.events }, dependencies),
      applyRefresh: (input: { accountId: string; database: D1Database; messageId: string; entries: EventRefreshEntry[] }): Promise<EventRefreshOutcome> =>
        applyEventRefreshWithGoogle(env, input.accountId, input.database, { messageId: input.messageId, entries: input.entries }, dependencies),
    },
    ruleRuns: {
      previewDraft: (input: { accountId: string; database: D1Database; messageId: string; ruleId: string }) =>
        previewSchemaDraftRule({ env, ...input }),
      startDraft: (input: { accountId: string; database: D1Database; messageId: string; ruleId: string; ruleRevision: number; extraction: MailExtraction }) =>
...
export const runAccountAutomation = async (
  env: Bindings,
  accountId: string,
  database: D1Database,
): Promise<AutomationSummary> => createAutomation(env).runAccount({ accountId, database });

/** @deprecated Prefer the Automation module interface at call sites. */
export const runEnabledAutomations = async (env: Bindings): Promise<void> => createAutomation(env).runEnabledAccounts();
