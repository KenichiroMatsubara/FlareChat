...
describe('Drive attachment publication', () => {
  it('returns a public link only after the Drive anyone-reader permission succeeds', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ data: 'cGRmLWJ5dGVz' }), { status: 200 }))
    ...
    await expect(publishDriveAttachment({
      ...
      attachment: { attachmentId: 'attachment-1', filename: 'agenda.pdf', mimeType: 'application/pdf', size: 9 },
    })).resolves.toEqual({ outcome: 'succeeded', driveFileId: 'drive-file-1', publicUrl: 'https://drive.example/file-1' });

    expect(fetchMock.mock.calls[1]?.[0]).toContain('upload/drive/v3/files?uploadType=multipart&fields=id%2CwebViewLink');
    expect(fetchMock.mock.calls[2]?.[0]).toBe('https://www.googleapis.com/drive/v3/files/drive-file-1/permissions');
    expect(JSON.parse(fetchMock.mock.calls[2]?.[1].body as string)).toEqual({ type: 'anyone', role: 'reader' });
    vi.unstubAllGlobals();
  });
  ...
  it('does not expose a link when public permission creation fails', async () => {
    ...
    await expect(publishDriveAttachment({
      accessToken: 'token', gmailMessageId: 'message-1', attachment: { attachmentId: 'attachment-1', filename: 'agenda.pdf', mimeType: 'application/pdf', size: 9 },
